public class QnA1 {
    public static void main(String[] args) {
        ...
        System.out.println("97~122사이의 무작위 난수: " + rand);
    }
}
/*
Q. final을 사용하니까 오히려 입력할게 많아지고 귀찮은데 왜 써야 하나요?
A. 예를 들어서 어떤 생산 라인(공장)을 관리하는 시스템이 있다고 가정한다.
...
final int SPECAL_PROCCESS = 6;

if(SPECAL_PROCCESS == num)으로 처리하면 된다.
나중에 추가 설비를 달면서 6번 공정을 바꾸고 이것을 8번으로 바꾼다 하더라도

final int SPECAL_PROCCESS = 8; 로 바꾸면 그만이다.
...
factory.process(vechicle, MAX - 2);
~~~~
...
max = 4; <<<--- 대규모 문제는 여기서 발생함
                ...
                다른 팀원들이 사용하는 값도
                무조건 MAX = 4로 처리되지 않을까?
                월내는 2,3,4~100등등 다양하게 들어가야하는데
                그냥 4를 떄려박으므로써 초토화가 발생함
                (갑자기 되던 것들이 전부 안돌아가기 시작함)
                서버가 뻗고, 사용자들이 탈출하고
                민원실로 전화가 솟구치고 매출이 감소하고
                야근이 발생하고 집에못가고 회사에 살림차리고
                빠르면 몇주 길면 몇달만에 문제를 찾아냄
                알고보니 MAX를 바꿔서 발생한 문제

                이것을 원천적으로 막을수 있다면 써야할까요 말아야 할까요?
