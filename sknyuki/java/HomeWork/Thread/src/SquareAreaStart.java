...
//------------------------------------------------------------------------------------//
        ...
        //사각형을 담당하는 클래스를 만든다.
        //가로변과 세로변의 전역변수를 만들어준다.
        //가로의 길이를 0.001로 나눈후 값을 array에 저장을 해준다
        //for문을 돌려주면서 세로 길이와 array의 인덱스들을 곱해준다
        //위 값을 더한다.

        //공유되는 critical Section 영역의 정보가 없다
        //=>가로길이 2를 1과 1로 구역을 나눠서 공통된 sum값에 thread로 추가해 가는 형식이라면?

        //수식을 짜보고나니 생각이나는 한계점
        //lock으로 for문을 각자 감싸고 있으면 결국 제어권이 넘어가는거는 한번뿐이 아닌가...
