public class ThreadCommon extends Thread {
    ...
    public ThreadCommon(int x, int y) { //parameter1, 2의 값이 x와 y가 됨.
        this.x = x;
        this.y = y;
    ...
    public static void calcRealTotal (int x) {
        realTotal = (int) (Math.ceil(x / dx)); //x(2)를 dx(0.001f)로 나눈 값을 올린 값 = realTotal
        System.out.println("realTotal = " + realTotal);

        totalRemain = realTotal % threadNum; //threadNum 값은 main에서 들어온 값으로 변경했음
